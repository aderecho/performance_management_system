    ...
    <q-dialog v-model="showFormDialog">
      <q-card class="role-dialog" style="width: 760px; max-width: 94vw">
        <q-card-section class="row items-center no-wrap q-gutter-sm">
          ...
          <div class="col">
            <div class="text-h6 leading-snug">{{ selectedRole ? 'Update Role' : 'Create Role' }}</div>
        ...
        <q-form @submit.prevent="handleSubmit">
          <q-card-section class="q-gutter-md">  
            ...
            <div class="perm-scroll">
              ...
              <div v-if="!formGroups.length" class="perm-empty column items-center justify-center text-grey-6">
    ...
    <q-dialog v-model="showViewDialog">
      <q-card class="role-dialog" style="width: 760px; max-width: 94vw">
        <q-card-section class="row items-center no-wrap q-gutter-sm">
          ...
          <div class="col min-w-0">
            <div class="text-h6 leading-snug ellipsis">{{ selectedRole?.name }}</div>
            <div class="text-caption text-grey-7">
              {{ viewTotalCount }} permission{{ viewTotalCount === 1 ? '' : 's' }} assigned
            </div>
          </div>
          <q-btn icon="close" flat round dense v-close-popup />
        </q-card-section>
        ...
        <q-card-section class="q-gutter-md">
          ...
          <div class="perm-scroll">
            ...
            <div v-if="!viewGroups.length" class="perm-empty column items-center justify-center text-grey-6">
              <ShieldOff :size="28" :stroke-width="1.5" />
              <div class="q-mt-sm text-caption">
                {{ viewTotalCount ? 'No permissions match your search.' : 'No permissions assigned to this role.' }}
              </div>
            </div>
          </div>
        </q-card-section>
...
import { FileText, ShieldCheck, ShieldOff, Search, SearchX, Check } from 'lucide-vue-next'
...
function groupPermissions(list, mode, search = '') {
  ...
  for (const permission of list) {
    if (needle) {
      const haystack = `${permission.name} ${permission.permission} ${permission.codename}`.toLowerCase()
...
function groupSelectedCount(group) {
  return group.permissions.reduce((count, permission) => count + (isSelected(permission.id) ? 1 : 0), 0)
}

function groupAllSelected(group) {
  return group.permissions.length > 0 && group.permissions.every((permission) => isSelected(permission.id))
}
